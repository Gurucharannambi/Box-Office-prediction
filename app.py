...
with left_col:
    ...
    if predict_clicked:
        ...
        predicted_val = predict(hero, director, genre, season, budget, screens, sequel)
        label, cat = categorise(predicted_val)
        conf = confidence_for(cat)
        display = f"Rs {predicted_val}Cr" if predicted_val < 1000 else f"Rs {predicted_val/100:.1f}Cr"
